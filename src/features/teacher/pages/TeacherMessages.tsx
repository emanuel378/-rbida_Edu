...
export default function TeacherMessages() {
  ...
  const myMessages = messages
    .filter(m => (!m.type || m.type === 'question') || m.targetType === 'question')
    .filter(m => {
  ...
  const getOrigin = (msg: any): Origin => {
    ...
    if (msg.targetType === 'question') {
      ...
      return {
        icon: HelpCircle,
        label: 'Questão',
        detail: `${msg.targetName || 'Questão'}${mod ? ` · Módulo "${mod.title}"` : ''}`,
    ...
    if (msg.lessonId) {
      ...
      return {
        icon: Video,
        label: 'Aula',
        detail: `${lesson?.title || 'Aula'} · Módulo "${mod?.title || ''}"`,
    ...
    return {
      icon: MessageCircle,
      label: 'Curso',
      detail: course?.title || 'Dúvida geral',
      ...
      <div className="p-4 sm:p-5 bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="min-w-0">
            ...
            <span className={`inline-flex items-center gap-1.5 mt-1.5 px-2.5 py-1 rounded-lg border text-xs font-medium ${origin.classes}`}>
              <OriginIcon className="w-3.5 h-3.5 flex-shrink-0" />
              <span className="truncate">{origin.label}: {origin.detail}</span>
            </span>
          </div>
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        ...
        <div className="p-5 bg-gradient-to-br from-blue-50 to-blue-50/50 rounded-2xl border border-blue-100 flex items-center justify-between">
          <div>
            <p className="text-3xl font-bold text-blue-700">{myMessages.length}</p>
      ...
      <div className="space-y-3">
        ...
        {replied.length === 0 ? (
          <div className="text-center py-10 bg-white rounded-2xl border border-gray-100">
            <Inbox className="w-10 h-10 text-gray-300 mx-auto mb-2" />
            <p className="text-gray-500 text-sm">Nenhuma mensagem respondida.</p>
          </div>
        ) : (
          <div className="space-y-3">
