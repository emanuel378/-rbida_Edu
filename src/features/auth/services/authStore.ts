...
export const useAuthStore = create<AuthState>((set, get) => ({
  ...
  loginWithEmail: async (email, password) => {
    if (!isSupabaseConfigured()) {
      return {
        error: 'Supabase não configurado. Use o login mockado.',
      }
    }
    ...
    if (data.user) {
      ...
      if (!profile) {
        ...
        const { data: created, error: profileError } =
          await supabase
            .from('profiles')
            .upsert(
              {
  ...
  signUp: async (
    ...
    if (!acceptedTerms) {
      return {
        error:
          'É necessário aceitar o Termo de Uso, a Política de Privacidade e a Política de Cancelamento e Reembolso.',
      }
    }
    ...
    if (data.user) {
      ...
      if (data.session) {
        // Usa upsert porque um trigger do Supabase ("handle_new_user") ou um
        // cadastro anterior já pode ter criado essa linha em profiles — um
        // INSERT puro então falharia com "duplicate key ... profiles_pkey".
